...
def crear_hotel(request):
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            hotel = Hotel(nombre = info_hotel["nombre"],ubicacion = info_hotel["ubicacion"],descripcion = info_hotel["descripcion"],desde = info_hotel["desde"],hasta = info_hotel["hasta"],precio = info_hotel["precio"])
...
def crear_vuelo(request):
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            vuelo = Vuelo(id_vuelo = info_vuelo["id_vuelo"],origen = info_vuelo["origen"],destino = info_vuelo["destino"],ida = info_vuelo["ida"],vuelta = info_vuelo["vuelta"],precio = info_vuelo["precio"])
...
def crear_excursion(request):
    ...
    if request.method == "POST":
        ...
        if formulario.is_valid():
            ...
            excursion = Excursion(nombre = info_excursion["nombre"],ubicacion = info_excursion["ubicacion"],descripcion = info_excursion["descripcion"],duracion = info_excursion["duracion"],precio = info_excursion["precio"])
